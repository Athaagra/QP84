...
for episode in range(episodes):
    ...
    while done!=True:
        steps+=1
        random_values=Q3[int(state_n[0])] + np.random.randint(11, size=(1,len(actions_list)))/1000
        actiona=np.argmax(random_values)
        print(actiona,np.argmax(state_n[0]))
        q_val=(int(np.argmax(state_n)),actiona)
        print('Print q_val {}'.format(q_val))
...
for episode in range(episodes):
    ...
    while done!=True:
        steps+=1
        random_values=Q4[int(state_n[0])] + np.random.randint(11, size=(1,len(actions_list)))/1000
        actiona=np.argmax(random_values)
        print(actiona,np.argmax(state_n[0]))
        q_val=(int(np.argmax(state_n)),actiona)
        print('Print q_val {}'.format(q_val))
        ...
        value=re + gamma * max(Q4[int(stat[0])])
        Q4[(q_val)]=value
        print('This is the reward {}'.format(re))
        reward_episode.append(re)
        state_n=stat
...
for episode in range(episodes):
    ...
    while done!=True:
        steps+=1
        random_values=Q5[int(state_n[0])] + np.random.randint(11, size=(1,len(actions_list)))/1000
        actiona=np.argmax(random_values)
        print(actiona,np.argmax(state_n[0]))
        q_val=(int(np.argmax(state_n)),actiona)
        print('Print q_val {}'.format(q_val))
        ...
        value=re + gamma * max(Q5[int(stat[0])])
        Q5[(q_val)]=value
        print('This is the reward {}'.format(re))
        reward_episode.append(re)
        state_n=stat
...
for episode in range(episodes):
    ...
    while done!=True:
        steps+=1
        random_values=Q6[int(state_n[0])] + np.random.randint(11, size=(1,len(actions_list)))/1000
        actiona=np.argmax(random_values)
        print(actiona,np.argmax(state_n[0]))
        q_val=(int(np.argmax(state_n)),actiona)
        print('Print q_val {}'.format(q_val))
        ...
        value=re + gamma * max(Q6[int(stat[0])])
        Q6[(q_val)]=value
        print('This is the reward {}'.format(re))
        reward_episode.append(re)
        state_n=stat
...
for episode in range(episodes):
    ...
    while done!=True:
        steps+=1
        random_values=Q7[int(state_n[0])] + np.random.randint(11, size=(1,len(actions_list)))/1000
        actiona=np.argmax(random_values)
        print(actiona,np.argmax(state_n[0]))
        q_val=(int(np.argmax(state_n)),actiona)
        print('Print q_val {}'.format(q_val))
        ...
        value=re + gamma * max(Q7[int(stat[0])])
        Q7[(q_val)]=value
        print('This is the reward {}'.format(re))
        reward_episode.append(re)
        state_n=stat
...
for episode in range(episodes):
    ...
    while done!=True:
        steps+=1
        random_values=Q8[int(state_n[0])] + np.random.randint(11, size=(1,len(actions_list)))/1000
        actiona=np.argmax(random_values)
        print(actiona,np.argmax(state_n[0]))
        q_val=(int(np.argmax(state_n)),actiona)
        print('Print q_val {}'.format(q_val))
        ...
        value=re + gamma * max(Q8[int(stat[0])])
        Q8[(q_val)]=value
        print('This is the reward {}'.format(re))
        reward_episode.append(re)
        state_n=stat
...
for _ in range(episodes):
    ...
    state_n3,actions3,data3,al_coun3,al_data3,bob_count3,bob_mail3,bob_mailbox3,bob_k3,done3,act_hist3,cum_re3,state_space3,action_space3,max_moves3,al_obs3,bob_data3,=reset(_)
    state_n4,actions4,data4,al_coun4,al_data4,bob_count4,bob_mail4,bob_mailbox4,bob_k4,done4,act_hist4,cum_re4,state_space4,action_space4,max_moves4,al_obs4,bob_data4,=reset(_)
    state_n5,actions5,data5,al_coun5,al_data5,bob_count5,bob_mail5,bob_mailbox5,bob_k5,done5,act_hist5,cum_re5,state_space5,action_space5,max_moves5,al_obs5,bob_data5,=reset(_)
    state_n6,actions6,data6,al_coun6,al_data6,bob_count6,bob_mail6,bob_mailbox6,bob_k6,done6,act_hist6,cum_re6,state_space6,action_space6,max_moves6,al_obs6,bob_data6,=reset(_)
    state_n7,actions7,data7,al_coun7,al_data7,bob_count7,bob_mail7,bob_mailbox7,bob_k7,done7,act_hist7,cum_re7,state_space7,action_space7,max_moves7,al_obs7,bob_data7,=reset(_)
    #state_n8,actions8,data8,al_coun8,al_data8,bob_count8,bob_mail8,bob_mailbox8,bob_k8,done8,act_hist8,cum_re8,state_space8,action_space8,max_moves8,al_obs8,bob_data8,=reset(_)
#    epochs, penalties, reward = 0, 0, 0
    ...
    while done1!=True or done2!=True or done3!=True: #or done4!=True:
        ...
        action3 = np.argmax(Q3[int(state_n3)])
        action4 = np.argmax(Q4[int(state_n4)])
        action5 = np.argmax(Q5[int(state_n5)])
        action6 = np.argmax(Q6[int(state_n6)])
        action7 = np.argmax(Q7[int(state_n7)])
        #action8 = np.argmax(Q8[int(state_n8)])
        #print(action)
        ...
        stat3,re3,do3,action_h3,bob_key3=step(action03,act_hist3,max_moves3,al_coun3,data3,al_data3,bob_count3,al_obs3,bob_k3,bob_data3,cum_re3,bob_mailbox3,bob_mail3,done3, verbose=0,)
        stat4,re4,do4,action_h4,bob_key4=step(action04,act_hist4,max_moves4,al_coun4,data4,al_data4,bob_count4,al_obs4,bob_k4,bob_data4,cum_re4,bob_mailbox4,bob_mail4,done4, verbose=0,)
        stat5,re5,do5,action_h5,bob_key5=step(action05,act_hist5,max_moves5,al_coun5,data5,al_data5,bob_count5,al_obs5,bob_k5,bob_data5,cum_re5,bob_mailbox5,bob_mail5,done5, verbose=0,)
        stat6,re6,do6,action_h6,bob_key6=step(action06,act_hist6,max_moves6,al_coun6,data6,al_data6,bob_count6,al_obs6,bob_k6,bob_data6,cum_re6,bob_mailbox6,bob_mail6,done6, verbose=0,)
        stat7,re7,do7,action_h7,bob_key7=step(action07,act_hist7,max_moves7,al_coun7,data7,al_data7,bob_count7,al_obs7,bob_k7,bob_data7,cum_re7,bob_mailbox7,bob_mail7,done7, verbose=0,)
        #stat8,re8,do8,action_h8,bob_key8=step(action08,act_hist8,max_moves8,al_coun8,data8,al_data8,bob_count8,al_obs8,bob_k8,bob_data8,cum_re8,bob_mailbox8,bob_mail8,done8, verbose=0,)
        done1=do1
        done2=do2
        done3=do3
        done4=do4
        ...
        #state_n8=stat8[0]
        #print(re)
        if do1 ==True or do2 == True or do3==True or do4==True or do5==True or do6==True or do7==True:
            if reward1==1 or reward2==1 or reward3==1 or reward4==1 or  reward5==1 or reward6==1 or  reward7==1:
                total_re.append(1)
                solved+=1
            #if re==1:
